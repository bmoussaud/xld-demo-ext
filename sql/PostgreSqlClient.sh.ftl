...
echo "${container.postgreSqlHome}/bin/psql" --set ON_ERROR_STOP=1 --dbname=${container.databaseName} --host=${hostname} --port=${container.port} --username=${cmn.lookup('username')} --no-password ${cmn.lookup('additionalOptions')!} 
